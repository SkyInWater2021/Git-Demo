...
export const Chart = function (Base) {
  class Basic extends Base {
    ...
    getChartOption() {
      const colors = ['#22DFFF', '#A869FE', '#FFBA00', '#61cec1']
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter: (e) => {
            ...
            e.forEach((item, index) => {
              ...
              const key = Object.keys(this.parserData.res)[seriesIndex]
              const label = this.parserData.res[key].label
              date = `<div>${this.parserData.xAxisData[dataIndex]}</div>`
              els += `<div>${marker} ${label}：${value}</div>`
            })
        ...
        legend: [
          {
            data: Object.keys(this.parserData.res).map(
              (key) => this.parserData.res[key].label
            ),
            top: 20,
        ...
        series: Object.keys(this.parserData.res).map((key, index) => {
          const color = colors[index % 3]
          return {
            name: this.parserData.res[key].label,
            type: 'line',
            showSymbol: false,
            ...
            data: this.parserData.res[key].data
          }
        })
      }
      return option
